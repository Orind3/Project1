...
public class PlayerRender extends Render {
    ...
    public void render(Graphics2D g2){
        if(this.getPlayState().getGamepanel().getKeyHandler().getkeypresses()[(int) 'W']||this.getPlayState().getGamepanel().getKeyHandler().getkeypresses()[(int) 'A']||this.getPlayState().getGamepanel().getKeyHandler().getkeypresses()[(int) 'D']||this.getPlayState().getGamepanel().getKeyHandler().getkeypresses()[(int) 'S']){
            if(this.getPlayState().getGamepanel().getGamestatemanager().getStates().size()==1)
            this.actionstate++;
            if(this.actionstate>29){
                this.actionstate = 0;
            }
        }
